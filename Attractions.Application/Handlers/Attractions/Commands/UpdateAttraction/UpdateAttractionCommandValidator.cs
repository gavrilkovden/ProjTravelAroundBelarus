...
namespace Attractions.Application.Handlers.Attractions.Commands.UpdateAttraction
{
    public class UpdateAttractionValidator : AbstractValidator<UpdateAttractionCommand>
    {
        ...
        {
            RuleFor(e => e.Id).GreaterThan(0).WithMessage("Id must be greater than 0.");
            RuleFor(e => e.Name).NotEmpty().WithMessage("Name is required.").MaximumLength(200).WithMessage("Name must be 200 characters or less.");
            RuleFor(e => e.Description).MaximumLength(1000).WithMessage("Description must be 1000 characters or less.");
            RuleFor(e => e.Price).GreaterThanOrEqualTo(0).When(x => x.Price.HasValue).WithMessage("Price must be greater than or equal to 0.");
            ...
            RuleFor(x => x.Address.Region).NotNull().NotEmpty().WithMessage("Region is required.");
