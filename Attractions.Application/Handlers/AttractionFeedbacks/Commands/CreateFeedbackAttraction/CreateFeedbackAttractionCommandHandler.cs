...
namespace Travel.Application.Handlers.Attractions.Commands.CreateFeedbackAttraction
{
    public class CreateFeedbackAttractionCommandHandler : IRequestHandler<CreateFeedbackAttractionCommand, GetFeedbackAttractionDto>
    {
        ...
        {
            var attraction = await _attractions.AsAsyncRead().Include(a => a.AttractionFeedback).FirstOrDefaultAsync(e => e.Id == request.AttractionId, cancellationToken);
            ...
            if (existingFeedback != null && existingFeedback.ValueRating.HasValue)
            {
                throw new ForbiddenException("You have already appreciated this attraction");
            }
